namespace ServiceStack.CodeGenerator.TypeScript.XmlDocumentationReader {
    ...
    public static class XmlDocumentationReader {
        ...
        private static XmlElement XMLFromName(Type type, char prefix, string name) {
            ...
            foreach (XmlElement xmlElement in xmlDocument["doc"]["members"]) {
                if (xmlElement.Attributes["name"].Value.Equals(fullName)) {
                    if (matchedElement != null) {
                        throw new MultipleMatchedElementsException("Multiple matches to query for " + type.Name + " - " + name, null);
                    }
            ...
            if (matchedElement == null) {
                throw new NoDocumentationFoundException("Could not find documentation for " + type.Name + " - " + name, null);
            }
        ...
        private static XmlDocument XMLFromAssemblyNonCached(Assembly assembly) {
            ...
            if (assemblyFilename.StartsWith(prefix)) {
                ...
                try {
                    ...
                    throw new FileNotFoundException("XML documentation not present for assembly " + assembly.FullName + " at " + exception.FileName + ".  Make sure that documentation is turned on for the project build.", exception);
                }
                ...
                throw new Exception("Could not ascertain assembly filename from " + assemblyFilename + " - prefix " + prefix, null);
            }
        }
    }
